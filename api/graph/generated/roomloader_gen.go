...
type RoomLoaderConfig struct {
	...
	Fetch func(keys []int) ([]*room.Room, []error)
...
func (l *RoomLoader) Load(key int) (*room.Room, error) {
...
func (l *RoomLoader) LoadThunk(key int) func() (*room.Room, error) {
	l.mu.Lock()
...
func (l *RoomLoader) LoadAll(keys []int) ([]*room.Room, []error) {
	results := make([]func() (*room.Room, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	rooms := make([]*room.Room, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *RoomLoader) LoadAllThunk(keys []int) func() ([]*room.Room, []error) {
	results := make([]func() (*room.Room, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *RoomLoader) Prime(key int, value *room.Room) bool {
	l.mu.Lock()
	var found bool
...
func (l *RoomLoader) unsafeSet(key int, value *room.Room) {
	if l.cache == nil {
		l.cache = map[int]*room.Room{}
	}
	l.cache[key] = value
}
