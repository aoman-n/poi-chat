...
func (l *UserLoader) Load(key int) (*user.User, error) {
...
func (l *UserLoader) LoadThunk(key int) func() (*user.User, error) {
	l.mu.Lock()
...
func (l *UserLoader) LoadAll(keys []int) ([]*user.User, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *UserLoader) LoadAllThunk(keys []int) func() ([]*user.User, []error) {
	results := make([]func() (*user.User, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *UserLoader) Prime(key int, value *user.User) bool {
	l.mu.Lock()
	var found bool
...
func (l *UserLoader) unsafeSet(key int, value *user.User) {
	if l.cache == nil {
		l.cache = map[int]*user.User{}
	}
	l.cache[key] = value
}
